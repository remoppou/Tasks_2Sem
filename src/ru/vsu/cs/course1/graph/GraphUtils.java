...
public class GraphUtils {
    ...
    public static String myToDot(Graph graph, List<GraphAlgorithms.Edge> edgeList) {
        ...
        for (int v1 = 0; v1 < graph.vertexCount(); v1++) {
            ...
            for (Integer v2 : graph.adjacencies(v1)) {
                String color = "";
                GraphAlgorithms.Edge myEdge = new GraphAlgorithms.Edge(v1,v2);
                if (edgeList.contains(myEdge)) {
                    color = "[color=red]";
                }
                sb.append(String.format("  %d %s %d %s", v1, (isDigraph ? "->" : "--"), v2, color)).append(nl);
                count++;
            }
            if (count == 0) {
